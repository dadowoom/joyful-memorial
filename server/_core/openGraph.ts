...
function getPublicSiteBase(req: Request) {
  const configured = process.env.PUBLIC_SITE_URL?.trim();
  if (configured) return configured.replace(/\/+$/, "");
  return `${getRequestOrigin(req)}${APP_BASE_PATH}`;
}

function getPath(req: Request, origin: string) {
  return stripBasePath(new URL(req.originalUrl || req.url || "/", origin).pathname);
}

function toPublicUrl(pathname: string, siteBase: string) {
  return new URL(
    pathname.replace(/^\/+/, ""),
    `${siteBase.replace(/\/+$/, "")}/`
  ).toString();
...
function toAbsoluteUrl(value: string | null | undefined, siteBase: string) {
  if (!value) return toPublicUrl(DEFAULT_IMAGE, siteBase);

  try {
    return new URL(value).toString();
  } catch {
    return toPublicUrl(value.startsWith("/") ? value : `/${value}`, siteBase);
  }
}
...
async function getMemorialMeta(req: Request): Promise<PageMeta | null> {
  ...
  const siteBase = getPublicSiteBase(req);
  ...
  const url = toPublicUrl(pathname, siteBase);
  ...
  if (isPrivate) {
    return {
      ...
      url,
      image: toAbsoluteUrl(DEFAULT_IMAGE, siteBase),
  ...
  return {
    ...
    description,
    url,
    image: toAbsoluteUrl(memorial.photoUrl || DEFAULT_IMAGE, siteBase),
...
export async function injectOpenGraphMeta(html: string, req: Request) {
  ...
  const defaultMeta: PageMeta = {
    ...
    url: toPublicUrl(pathname, siteBase),
    image: toAbsoluteUrl(DEFAULT_IMAGE, siteBase),
    imageAlt: SERVICE_TITLE,
  };
