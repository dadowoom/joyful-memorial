...
- `/kiosk`: 세로형 검색 화면
- `/kiosk/memorial/{slug}`: 키오스크 전용 상세 화면
- 일반 웹 상세를 그대로 쓰지 않는다.
- 세로 화면에서 하단 여백이 충분해야 한다.
- 미사용 시 첫 화면 복귀 기능을 고려한다.
...
curl -sS -o /dev/null -w "KIOSK:%{http_code}\n" http://115.68.224.123:3070/kiosk
```
